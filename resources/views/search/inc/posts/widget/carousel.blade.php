...
@if ($totalPosts > 0)
	...
	@endif
	<div class="container{{ $isFromHome ? '' : ' my-3' }}{{ $hideOnMobile }} bg-white" style="padding: 0px;border-radius: 4px;">
		<div class="col-xl-12 content-box layout-section" style="border-radius: 4px;">
					...
					<div class="inner">
						<h2 style="padding-left: 15px;">
							...
							@php
								...
								$parentCategory = \App\Models\Category::find($categoryId);

								if($parentCategory && isset($parentCategory->parent->id)){
									// Have sub category.

									$parentCategoryId = $parentCategory->parent->id;

									$urlArray['id'] = $parentCategoryId;

									// $urlArray['subId'] = $categoryId;

								}else{
									// Not sub category.

									$urlArray['id'] = $categoryId;
								}

								$url = route('search', ['c' => base64_encode(json_encode($urlArray))]);

							@endphp
							<a href="{{ $url }}" class="sell-your-item">
				...
				<div class="relative content featured-list-row clearfix">
					...
					<div class="large-12 columns">
						<div class="no-margin featured-list-slider">
							...
							<div class="row item-list-container-eke result-body">
								
								@includeFirst([config('larapen.core.customizedViewPath') . 'search.inc.posts.template.sublist', 'search.inc.posts.template.sublist'])
