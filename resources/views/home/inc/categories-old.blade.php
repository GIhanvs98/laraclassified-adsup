...
<div class="container{{ $hideOnMobile }}">
	<div class="col-xl-12 content-box layout-section categories-layout-section">
				...
				@if (!empty($categories))
					@foreach($categories as $key => $cat)
						<div class="col-md-3 col-sm-4 col-6 f-category">
							<a href="{{ route('search') }}/?c={{ base64_encode(json_encode(['id' => $cat['id'] ])) }}">
								<div class="row no-gutters">
									@if (in_array(config('settings.list.show_category_icon'), [2, 6, 7, 8]))
										<!--i class="col-12 col-md-3 {{ data_get($cat, 'icon_class') ?? 'fas fa-folder' }}"></i-->
									@endif
									<img src="{{ data_get($cat, 'picture_url') }}" class="col lazyload img-fluid" alt="{{ data_get($cat, 'name') }}">
									<div class="col">
										<div class="row mt-1 text-center text-md-start">
											<div class="col-12">
												<span>{{ data_get($cat, 'name') }}</span>
											</div>
											@if (config('settings.list.count_categories_listings'))
												<div class="col-12">
													<span class="text-black-50">{{ $countPostsPerCat[data_get($cat, 'id')]['total'] ?? 0 }}&nbsp;ads</span>
												</div>
											@endif
										</div>
									</div>
