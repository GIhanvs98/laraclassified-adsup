...
class FrontController extends Controller
{
	...
	private function getUserMenu(): \Illuminate\Support\Collection
	{
		...
		$membership = [
			'name' => 'My Membership',
			'url' => url('account/membership'),
			'icon' => 'bi bi-person-check',
			'group' => t('My Account'),
			'countVar' => null,
			'inDropdown' => false,
			'isActive' => (request()->segment(2) == 'membership'),
		];

		$shop = [
			'name' => 'Shop Page',
			'url' => url('account/shop'),
			'icon' => 'bi bi-shop',
			'group' => 'My Account',
			'countVar' => null,
			'inDropdown' => false,
			'isActive' => (request()->segment(2) == 'shop'),
		];

		$adsReports = [
			'name' => 'Customer Ads Reports',
			'url' => url('account/ads-reports'),
			'icon' => 'bi bi-exclamation-triangle',
			'group' => 'Ads Reports',
			'countVar' => null,
			'inDropdown' => false,
			'isActive' => (request()->segment(2) == 'ads-reports'),
		];

		// array_push($menuArray, $membership, $shop, $adsReports, $logOut, $closeAccount);
		array_push($menuArray, $membership, $shop, $adsReports);
