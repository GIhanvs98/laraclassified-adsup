...
class CreateController extends FrontController
{
	...
	public function getPostStep(Request $request)
	{
		...
		return appView('post.createOrEdit.multiSteps.create', compact('postInput', 'categoryGroups'));
	}
	...
	public function postPostStep(PostRequest $request)
	{
		...
		$category_parent_id = Category::find($request->category_id)->parent_id;

		/* -------------------- Check the post limit -------------------- */

		$postsCount = Post::where('user_id', auth()->user()->id)->where('category_id', $category_parent_id )->count();
		...
		if (isset(auth()->user()->membership)) {
			...
			if( ($adsLimit > $postsCount) || !isset($adsLimit) ){
				...
				return view('post.createOrEdit.multiSteps.category-validation');
			}
		}else{
	...
	// dd('aaaaaaaaaaaaaa');
		...
		// dd($payment);
		$topPackge = DB::table('packages')->where("packge_type", "Top ads")->where("active", 1)->get()->toArray();
		$bumpPackge = DB::table('packages')->where("packge_type", "Bump Ads")->where("active", 1)->get()->toArray();
		
		return appView('post.createOrEdit.multiSteps.packages.create', compact('payment'), ['topAds' => $topPackge, 'bumpAds' => $bumpPackge]);
	}
