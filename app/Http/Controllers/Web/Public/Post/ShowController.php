...
class ShowController extends FrontController
{
	...
	public function index()
	{
		...
		if (Post::whereId($postId)->doesntExist()) {
		...
		if (isset(auth()->user()->is_admin) && auth()->user()->is_admin == "1" && !isset($ad->reviewed_at) && empty($ad->reviewed_at) /* $ad->is_approved == "0" */) {

			$isReviewing = true;

		} else if (auth()->check() && (auth()->user()->id == $ad->user_id) && !isset($ad->reviewed_at) && empty($ad->reviewed_at) /* $ad->is_approved == "0" */) {

			$isReviewing = true;

		} else if (request()->session()->has('guest-user') && !isset($ad->reviewed_at) && empty($ad->reviewed_at) /* $ad->is_approved == "0" */) {

			$guestUser = session('guest-user');

			$isReviewing = true;

			if ($guestUser->doesntHavePost($ad->id)) {
				// Unauthrozed.
				return abort(401);
			} else {

				if ($guestUser->havePost($ad->id) && !request()->isSecure()) {
			...
			if (!isset($ad->reviewed_at) && empty($ad->reviewed_at) /* $ad->is_approved == "0" */) {
