	...
	private function getUnorderedCatBreadcrumb($cat, int &$position = 0, array &$tab = []): array
	{
		...
		if (empty($tab)) {
			$tab[] = [
				...
				'url'      => route('search', ['c' => base64_encode(json_encode([ "id" => data_get($cat, 'id') ])) ]),
				'position' => $position,
			];
		}
		...
		if (!empty(data_get($cat, 'parent'))) {
			$tab[] = [
				...
				'url'      => route('search', ['c' => base64_encode(json_encode([ "id" => data_get($cat, 'id'), "subId" => data_get($cat, 'parent.id') ])) ]),
				'position' => $position + 1,
			];
