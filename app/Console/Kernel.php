...
class Kernel extends ConsoleKernel
{
	...
	{
		// Deactivate expired ad-promotions.
		$schedule->command('check:ad-promotions')
			->everyFiveMinutes()
			->runInBackground()
			->withoutOverlapping()
		...
		$schedule->command('check:memberships')
			...
			->runInBackground()
			->withoutOverlapping()
			->appendOutputTo("./storage/logs/check-memberships.log");

		// Sheduling bump ads for next day.
		$schedule->command('run:bump-ad')
			->everyTwoMinutes()
			->runInBackground()
			->withoutOverlapping()
			->appendOutputTo("./storage/logs/run-bump-ad.log");
			
		// Delete review violated ads.
		$schedule->command('delete:review-violated-ads')
			->everyThreeHours() # Chnage this if the db load increases.
			->runInBackground()
			->withoutOverlapping()
			->appendOutputTo("./storage/logs/delete-review-violated-ads.log");

		// Delete expired ads.
		$schedule->command('delete:expired-ads')
			->everySixHours() # Chnage this if the db load increases.
			->runInBackground()
			->withoutOverlapping()
			->appendOutputTo("./storage/logs/delete-expired-ads.log");

		// Remove thumbnail-images without pictures and pictures without posts
		$schedule->command('cleanup:orphans')
			->monthly() # Chnage this if the db load increases.
			->runInBackground()
			->withoutOverlapping()
			->appendOutputTo("./storage/logs/files-cleanup.log");

		$schedule->command('queue:work --stop-when-empty')
			->runInBackground()
			->everyMinute()
			->withoutOverlapping(15*60)
			->appendOutputTo("./storage/logs/queue-work.log");
