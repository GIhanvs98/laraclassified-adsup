...
class AdminSettingsGeneralSearchKeywords extends Component
{
    ...
    {
        return view('livewire.admin-settings-general-search-keywords', [
            'searchKeywords' => SearchKeyword::withWhereHas('category')->where('keywords', 'like', '%' . $this->search . '%')->get(),
            'searchKeywordsCount' => SearchKeyword::withWhereHas('category')->count(),
        ]);
    }

    public function submit()
    {
        if (SearchKeyword::withWhereHas('category')->count() < 4) {

            $this->addError('keywords', 'Only 4 keywords are allowed!');
